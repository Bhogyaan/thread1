...
export default function LoginCard({ isAdmin = false }) {
	...
	const handleLogin = async () => {
		...
		try {
			...
			console.log("Logged in user:", data); // Debug log
					...
					<Stack spacing={4}>
						<FormControl isRequired>
							...
							<Input
								...
								onChange={(e) =>
									!isAdmin && setInputs((inputs) => ({ ...inputs, username: e.target.value }))
								}
								isDisabled={isAdmin}
							/>
						</FormControl>
						<FormControl isRequired>
							...
							<InputGroup>
								<Input
									...
									onChange={(e) =>
										!isAdmin && setInputs((inputs) => ({ ...inputs, password: e.target.value }))
									}
									isDisabled={isAdmin}
								/>
